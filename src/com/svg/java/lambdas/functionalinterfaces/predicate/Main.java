...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Predicate<Person> pName2 = (p) -> p.getName().contentEquals("name2");
        personList.stream().filter(pName2).forEach(System.out::println);

        System.out.println("\nPersons with name2 or name3:");
        Predicate<Person> pName2OrName3 = pName2.or((p) -> p.getName().contentEquals("name3"));
        personList.stream().filter(pName2OrName3).forEach(System.out::println);

        System.out.println("\nPersons with name2 or name3 whose are retired:");
        Predicate<Person> pName2OrName3AndAreRetired = pName2.or((p) -> p.getName().contentEquals("name3")).and((p) -> p.isRetired());
